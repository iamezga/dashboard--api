...
export class PostgresRoleRepository implements RoleRepositoryInterface {
	...
	private mapPrismaRoleToDomain(prismaRole: PrismaRoleModel): Role {
		return {
			...
			config: prismaRole.config as object,
	 ...
	 * @param {PrismaRoleModel & { rolePermissions: ({ permission: PrismaPermissionModel })[] }} prismaRoleWithPermissions - Role object with nested permissions from Prisma.
	 * @returns {RoleWithPermissions} The mapped domain RoleWithPermissions entity.
	 */
	private mapPrismaRoleWithPermissionsToDomain(
		prismaRoleWithPermissions: PrismaRoleModel & {
			rolePermissions: { permission: PrismaPermissionModel }[]
		}
	): RoleWithPermissions {
	...
	async findByIdWithPermissions(
		...
		const prismaRole = await this.db.role.findUnique({
			...
			include: {
				rolePermissions: {
					include: {
						permission: true // Include the full permission entity
					}
				}
			}
		})
